...
class Surveys extends Member_Controller
{
				...
				if (!empty($survey_item)) {
					...
					if (count($completed_items) <= intval($survey_item['limit_per_user'])) {
						...
						if (!array_key_exists($question_id, $completed_questions)) {
							...
							$completed_questions[$question_id] = $options;
	...
	public function single($slug = null)
	{
		...
		if (in_array('completeQuestion', $this->permission)) {
			// render view
			$survey_item = $this->model_surveys->getSurveyItemBySlug($slug);
			$this->data['title'] = $survey_item['title'] . " | SurveyVine!";
	...
	public function completed($slug = null)
	{
		...
		if (!empty($survey_item) && !empty($completed_items)) {
			...
			$this->data['title'] = "Claim your rewards - " . $survey_item['title'] . " | SurveyVine!";
			$this->data['survey_item'] = $survey_item;
	...
	public function results($slug)
	{
		...
		$survey_item = $this->model_surveys->getSurveyItemBySlug($slug);
		if (!empty($survey_item)) {
			$total_completed = count($this->model_surveys->getCompletedBySurveyId(null, $survey_item['id']));
			// chart div elems
			$chart_elems = array();
			$survey_questions = $this->model_surveys->getSurveyQuestionsById($survey_item['id']);
			$this->data['survey_questions'] = $survey_questions;

			// render view
			$this->data['survey_item'] = $survey_item;
			$this->render_admin('pages/admin/activities/surveys/result', $this->data);
		} else {
			redirect('surveys/admin', 'refresh');
		}
	}

	public function fetchSurveyResultsChart($id)
	{
		$result = array();

		$survey_item = $this->model_surveys->getSurveyItemById($id);
		if (!empty($survey_item)) {
			$results_arr = $this->create_survey_options_index($survey_item);
			// get completed items all - array(question[options]...)
			$completed_items = $this->model_surveys->getCompletedBySurveyId(null, $survey_item['id']);
			if (!empty($completed_items)) {
				foreach ($completed_items as $comp_item) {
					$completed_questions = unserialize($comp_item['questions']);
					foreach ($completed_questions as $key => $comp_questx) { //questions each
						foreach ($comp_questx as $value) { //options each
							$results_arr[$key][$value] += 1;
						}
					}
				}
			}
			$result = $this->serialize_result($results_arr, $survey_item);
		}

		echo json_encode($result);
	}

	public function serialize_result($chart_x, $survey_item) {
		$result = array();
		if (!empty($chart_x)) {
			// get surveyitem, get questions, get options
			// foreach question create new array push options value
			$survey_questions = $this->model_surveys->getSurveyQuestionsById($survey_item['id']);
			if (!empty($survey_questions)) {
				foreach ($survey_questions as $question) {
					$data = array('question' => str_replace(" ", "-", $question['question_text']), 'options' => array());
					$questx_options = $this->model_surveys->getSurveyOptionsByQuestionId($question['id']);
					if (!empty($questx_options)) {
						foreach ($questx_options as $key => $optionx) {
							$value = $chart_x[$question['id']][$optionx['id']];
							$data['options'][$key] = array('label' => $optionx['option_text'], 'value' => $value);
						}
					}
	...
	public function create_survey_options_index($survey_item)
	{
		$results_arr = array();

		$survey_questions = $this->model_surveys->getSurveyQuestionsById($survey_item['id']);
		if (!empty($survey_questions)) {
			foreach ($survey_questions as $questx) {
				$questx_options = $this->model_surveys->getSurveyOptionsByQuestionId($questx['id']);
				if (!empty($questx_options)) {
					$options_arr = array();
					foreach ($questx_options as $optionx) {
						$options_arr[$optionx['id']] = 0;
					}
					$results_arr[$questx['id']] = $options_arr;
				}
			}
		}
	...
	public function readSurveyItems()
	{
		...
		foreach ($items as $key => $value) {
			...
			if (in_array('manageSurvey', $this->permission)) {
				...
				$buttons .= "<a href='" . base_url('surveys/results/' . $value['slug']) . "' title='results' class='btn btn-default'><i class='ion ion-podium'></i></a>";
			}
	...
	public function create()
	{
		...
		if (in_array('createSurvey', $this->permission)) {
			...
			if ($this->form_validation->run() == TRUE) {
				...
				$slug = slugify('sv-' . rand(0000, 9999) . '-' . substr($survey_title, 0, 25));
			...
			if (!empty($survey_item)) {
				...
				if ($this->form_validation->run() == TRUE) {
					...
					if ($updated_item) {
						...
						$this->model_logs->logActivity($log);
						// flashdata
						$this->session->set_flashdata('alert', array('classname' => 'alert-success', 'title' => 'Item Updated', 'message' => 'Updated Survey Item'));
					} else {
		...
		if (in_array('manageSurvey', $this->permission)) {
			...
			if ($survey_item['status'] == 'draft') {
				if ($survey_item['created_by'] == $user_id) {
					...
					$this->session->set_flashdata('alert', array('classname' => 'alert-warning', 'title' => 'Error Occurred', 'message' => 'Not Item Owner'));
					redirect('surveys/edit/' . $survey_slug, 'refresh');
				}
			} else {
				redirect('surveys/admin', 'refresh');
	...
	public function deleteQuestion($question_id = null)
	{
		...
		if (in_array('manageSurvey', $this->permission)) {
			...
			redirect('surveys/edit/' . $survey_item['slug'], 'refresh');
		}
	}
