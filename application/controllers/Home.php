...
class Home extends Member_Controller
{
	...
	public function change_currency()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			// get config value by name and set session data;
			$currency_data = $this->model_config->getConfigByName($value);
			if (!empty($currency_data)) {
				$this->session->set_userdata('currency', array('currency' => $currency_data['value'], 'rate' => $currency_data['extra']));
			}
		}
